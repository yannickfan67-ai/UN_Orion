...
static void draw_window_content(int app){Window*w=&wins[app];if(app==APP_TERMINAL)draw_terminal(w);else if(app==APP_FILES)draw_files(w);else if(app==APP_NOTES)draw_notes(w);else if(app==APP_PAINT)draw_paint(w);else draw_about(w);}
...
static void handle_press(void){
    ...
    if(start_open){int sx=14,sy=ty-344;for(int i=0;i<APP_COUNT;i++)if(inside(mouse_x,mouse_y,sx+10,sy+70+i*45,260,38)){focus_app(i);return;}if(inside(mouse_x,mouse_y,sx+10,sy+300,260,32)){gfx_rect(0,0,gfx_width(),gfx_height(),0x05080B);gfx_display_text(80,90,"GOODBYE",0xDCE7F2);__asm__ volatile("cli");for(;;)__asm__ volatile("hlt");}start_open=0;redraw();return;}
...
static void terminal_exec(void){serial_write("desktop shell: ");serial_write(term_cmd);serial_write("\r\n");if(term_len){char e[TERM_LEN]="orion > ";append(e,term_cmd,sizeof(e));term_push(e);}if(streq(term_cmd,"help"))term_push("help clear info mem alloc uptime desktop");else if(streq(term_cmd,"clear"))term_count=0;else if(streq(term_cmd,"info"))term_push("UN_Orion Desktop 0.0.4 / window manager + PS/2 mouse");else if(streq(term_cmd,"desktop"))term_push("Terminal, Files, Notes and Paint are native kernel apps.");else if(streq(term_cmd,"mem")){char s[TERM_LEN]="free physical memory ",n[24];u64dec(pmm_free_pages()/256,n);append(s,n,sizeof(s));append(s," MB",sizeof(s));term_push(s);}else if(streq(term_cmd,"alloc")){uint64_t p=pmm_alloc_page();char s[TERM_LEN]="allocated ",h[19];hex64(p,h);append(s,h,sizeof(s));term_push(s);}else if(streq(term_cmd,"uptime")){char s[TERM_LEN]="uptime ",n[24];u64dec(timer_frequency()?timer_ticks()/timer_frequency():0,n);append(s,n,sizeof(s));append(s," seconds",sizeof(s));term_push(s);}else if(term_len)term_push("unknown command");term_len=0;term_cmd[0]=0;redraw();}
static void key_ascii(char c){if(active_app==APP_TERMINAL){if(term_len<(int)sizeof(term_cmd)-1){term_cmd[term_len++]=c;term_cmd[term_len]=0;redraw();}}else if(active_app==APP_NOTES){if(note_len<NOTE_CAP-1){notes[note_len++]=c;notes[note_len]=0;redraw();}}}
void desktop_key_scancode(uint8_t s){
    ...
    if(s==0x1C){if(active_app==APP_TERMINAL)terminal_exec();else if(active_app==APP_NOTES&&note_len<NOTE_CAP-1){notes[note_len++]='\n';notes[note_len]=0;redraw();}return;}
    if(s==0x0E){if(active_app==APP_TERMINAL&&term_len){term_cmd[--term_len]=0;redraw();}else if(active_app==APP_NOTES&&note_len){notes[--note_len]=0;redraw();}return;}
...
void desktop_init(OrionBootInfo *bi){
    g_bi=bi;(void)g_bi;wins[APP_TERMINAL]=(Window){280,130,720,480,1,0,11,"Terminal"};wins[APP_FILES]=(Window){230,115,700,440,0,0,2,"Files"};wins[APP_NOTES]=(Window){330,125,610,480,0,0,3,"Notes"};wins[APP_PAINT]=(Window){250,90,760,560,0,0,4,"Paint"};wins[APP_ABOUT]=(Window){380,165,540,360,0,0,5,"About UN_Orion"};active_app=APP_TERMINAL;ztop=11;term_push("Welcome to UN_Orion Desktop 0.0.4.");term_push("Use the mouse, open apps, drag windows, and try Paint.");term_push("Type 'help' for terminal commands.");redraw();
}

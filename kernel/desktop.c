...
static char browser_url[128]=""; static int browser_url_len;
static char browser_page[4096]="Orion Browser\n\nType an HTTP address above and press Enter.\nExample: 10.0.2.2/";
static char browser_status[96]="Start page";
...
static void draw_desktop_icons(void){icon_box(26,42,"Terminal",APP_TERMINAL);icon_box(26,133,"Files",APP_FILES);icon_box(26,224,"Notes",APP_NOTES);icon_box(26,315,"Paint",APP_PAINT);icon_box(26,406,"Browser",APP_BROWSER);icon_box(26,497,"Network",APP_NETWORK);}
...
static void draw_taskbar(void){
    ...
    for(int i=0;i<APP_COUNT;i++){int x=70+i*48;if(wins[i].open&&!wins[i].minimized)gfx_rect(x,y+8,40,38,active_app==i?0x263D59:0x172638);else gfx_rect(x,y+8,40,38,0x111C2A);gfx_text(x+13,y+11,i==0?">":i==1?"F":i==2?"N":i==3?"P":i==4?"B":i==5?"W":"i",0xDDE8F3,1);}
...
static void draw_start_menu(void){
    ...
    const char*names[APP_COUNT]={"Terminal","Files","Notes","Paint","Browser","Network","About"};for(int i=0;i<APP_COUNT;i++){int iy=y+68+i*43;if(inside(mouse_x,mouse_y,x+10,iy,w-20,36))gfx_rect(x+10,iy,w-20,36,0x203A56);gfx_text(x+24,iy+4,names[i],0xE4EDF6,1);}
...
static void draw_browser(Window*w){int bx=w->x+1,by=w->y+TITLE_H+1,bw=w->w-2,bh=w->h-TITLE_H-2;gfx_rect(bx,by,bw,bh,0xF5F7FA);gfx_rect(bx,by,bw,50,0xDEE6EE);gfx_rect(bx+14,by+9,bw-92,32,0xFFFFFF);gfx_text(bx+24,by+9,browser_url_len?browser_url:"Enter http address...",browser_url_len?0x26394B:0x8C9AAA,1);gfx_rect(bx+bw-68,by+9,54,32,0x3578B7);gfx_text(bx+bw-55,by+9,"Go",0xFFFFFF,1);gfx_text(bx+16,by+58,browser_status,0x65788A,1);int x=bx+18,y=by+94,col=0;char one[2]={0,0};for(int i=0;browser_page[i]&&y<by+bh-24;i++){char c=browser_page[i];if(c=='\n'||col>=72){y+=22;x=bx+18;col=0;if(c=='\n')continue;}one[0]=c;uint32_t adv=gfx_text((uint32_t)x,(uint32_t)y,one,0x26394B,1);x+=(int)adv;col++;}}
static void draw_network(Window*w){int bx=w->x+1,by=w->y+TITLE_H+1,bw=w->w-2,bh=w->h-TITLE_H-2;gfx_rect(bx,by,bw,bh,0xF0F4F8);gfx_text(bx+24,by+20,"Network",0x233A50,1);gfx_text(bx+24,by+58,net_ready()?"Connected":"No supported adapter",net_ready()?0x2A7B4F:0xA04455,1);char s[96]="Adapter: ";append(s,net_driver_name(),sizeof(s));gfx_text(bx+24,by+98,s,0x5C7184,1);uint8_t ip[4];char v[32];net_get_ipv4(ip);iptext(ip,v,sizeof(v));s[0]=0;append(s,"IPv4: ",sizeof(s));append(s,v,sizeof(s));gfx_text(bx+24,by+132,s,0x5C7184,1);net_get_gateway(ip);iptext(ip,v,sizeof(v));s[0]=0;append(s,"Gateway: ",sizeof(s));append(s,v,sizeof(s));gfx_text(bx+24,by+166,s,0x5C7184,1);char n[24];s[0]=0;append(s,"Packets RX/TX: ",sizeof(s));u64dec(net_rx_packets(),n);append(s,n,sizeof(s));append(s," / ",sizeof(s));u64dec(net_tx_packets(),n);append(s,n,sizeof(s));gfx_text(bx+24,by+200,s,0x5C7184,1);gfx_rect(bx+24,by+248,170,36,0x2D6FA8);gfx_text(bx+45,by+250,"Ping gateway",0xFFFFFF,1);if(net_ping_result>=0)gfx_text(bx+220,by+250,net_ping_result?"Reply received":"No reply",net_ping_result?0x2A7B4F:0xA04455,1);gfx_text(bx+24,by+310,"Early stack: RTL8139 / ARP / IPv4 / ICMP / UDP / DNS / TCP / HTTP",0x667C90,1);}
static void draw_about(Window*w){int bx=w->x+1,by=w->y+TITLE_H+1,bw=w->w-2,bh=w->h-TITLE_H-2;gfx_rect(bx,by,bw,bh,0xF0F4F8);gfx_display_text(bx+24,by+22,"ORION",0x1E4267);gfx_text(bx+28,by+93,"UN_Orion Desktop " VERSION,0x273A4D,1);gfx_text(bx+28,by+126,"From-scratch x86_64 kernel",0x63768A,1);gfx_text(bx+28,by+162,mouse_available()?"Mouse: PS/2 IRQ12 active":"Mouse: unavailable",0x63768A,1);char s[64]="Free physical memory: ",n[24];u64dec(pmm_free_pages()/256,n);append(s,n,sizeof(s));append(s," MB",sizeof(s));gfx_text(bx+28,by+195,s,0x63768A,1);gfx_text(bx+28,by+228,"Traf Typeface v2.1",0x63768A,1);}
...
static void handle_press(void){
    ...
    int app=top_window_at(mouse_x,mouse_y);if(app>=0){Window*w=&wins[app];w->z=++ztop;active_app=app;if(inside(mouse_x,mouse_y,w->x+w->w-40,w->y+5,34,31)){w->open=0;active_app=-1;redraw();return;}if(inside(mouse_x,mouse_y,w->x,w->y,w->w,TITLE_H)){dragging=app;drag_dx=mouse_x-w->x;drag_dy=mouse_y-w->y;redraw();return;}if(app==APP_PAINT){int bx=w->x+1,by=w->y+TITLE_H+1;if(inside(mouse_x,mouse_y,bx+96,by+8,60,30)){for(int i=0;i<PAINT_W*PAINT_H;i++)paint[i]=0;redraw();return;}cursor_restore();paint_at(mouse_x,mouse_y);cursor_refresh();return;}if(app==APP_BROWSER){int bx=w->x+1,by=w->y+TITLE_H+1,bw=w->w-2;if(inside(mouse_x,mouse_y,bx+bw-68,by+9,54,32)&&browser_url_len){scopy(browser_status,sizeof(browser_status),"Loading...");redraw();if(net_http_get(browser_url,browser_page,sizeof(browser_page),browser_status,sizeof(browser_status))){}redraw();return;}}if(app==APP_NETWORK){int bx=w->x+1,by=w->y+TITLE_H+1;if(inside(mouse_x,mouse_y,bx+24,by+248,170,36)){net_ping_result=net_ping_gateway(1800);redraw();return;}}redraw();return;}
    active_app=-1;redraw();
}
...
static void terminal_exec(void){serial_write("desktop shell: ");serial_write(term_cmd);serial_write("\r\n");if(term_len){char e[TERM_LEN]="orion > ";append(e,term_cmd,sizeof(e));term_push(e);}if(streq(term_cmd,"help"))term_push("help clear info mem alloc uptime desktop net ping browser nettest openwrt");else if(streq(term_cmd,"clear"))term_count=0;else if(streq(term_cmd,"info"))term_push("UN_Orion Desktop 0.0.5 / desktop + network + browser");else if(streq(term_cmd,"desktop"))term_push("Terminal, Files, Notes, Paint, Browser and Network are native apps.");else if(streq(term_cmd,"mem")){char s[TERM_LEN]="free physical memory ",n[24];u64dec(pmm_free_pages()/256,n);append(s,n,sizeof(s));append(s," MB",sizeof(s));term_push(s);}else if(streq(term_cmd,"alloc")){uint64_t p=pmm_alloc_page();char s[TERM_LEN]="allocated ",h[19];hex64(p,h);append(s,h,sizeof(s));term_push(s);}else if(streq(term_cmd,"uptime")){char s[TERM_LEN]="uptime ",n[24];u64dec(timer_frequency()?timer_ticks()/timer_frequency():0,n);append(s,n,sizeof(s));append(s," seconds",sizeof(s));term_push(s);}else if(streq(term_cmd,"net")){char s[TERM_LEN]="network ";append(s,net_ready()?"connected / RTL8139 / 10.0.2.15":"offline",sizeof(s));term_push(s);}else if(streq(term_cmd,"ping")){term_push(net_ping_gateway(1500)?"gateway reply received":"gateway no reply");}else if(streq(term_cmd,"nettest")){char st[64];if(net_http_get("10.0.2.2:18080/",browser_page,sizeof(browser_page),st,sizeof(st))){term_push("HTTP test success / page loaded into Browser");serial_write("HTTP test success\r\n");}else{term_push(st);serial_write("HTTP test failed\r\n");}}else if(streq(term_cmd,"openwrt")){static const uint8_t ip[4]={192,168,1,2},mask[4]={255,255,255,0},gw[4]={192,168,1,1},dns[4]={192,168,1,1};net_configure(ip,mask,gw,dns);term_push("network profile: OpenWrt LAN 192.168.1.2/24");}else if(streq(term_cmd,"browser")){focus_app(APP_BROWSER);term_len=0;term_cmd[0]=0;return;}else if(term_len)term_push("unknown command");term_len=0;term_cmd[0]=0;redraw();}
static void key_ascii(char c){if(active_app==APP_TERMINAL){if(term_len<(int)sizeof(term_cmd)-1){term_cmd[term_len++]=c;term_cmd[term_len]=0;redraw();}}else if(active_app==APP_NOTES){if(note_len<NOTE_CAP-1){notes[note_len++]=c;notes[note_len]=0;redraw();}}else if(active_app==APP_BROWSER){if(browser_url_len<(int)sizeof(browser_url)-1){browser_url[browser_url_len++]=c;browser_url[browser_url_len]=0;redraw();}}}
void desktop_key_scancode(uint8_t s){
    ...
    if(s==0x1C){if(active_app==APP_TERMINAL)terminal_exec();else if(active_app==APP_NOTES&&note_len<NOTE_CAP-1){notes[note_len++]='\n';notes[note_len]=0;redraw();}else if(active_app==APP_BROWSER&&browser_url_len){scopy(browser_status,sizeof(browser_status),"Loading...");redraw();net_http_get(browser_url,browser_page,sizeof(browser_page),browser_status,sizeof(browser_status));redraw();}return;}
    if(s==0x0E){if(active_app==APP_TERMINAL&&term_len){term_cmd[--term_len]=0;redraw();}else if(active_app==APP_NOTES&&note_len){notes[--note_len]=0;redraw();}else if(active_app==APP_BROWSER&&browser_url_len){browser_url[--browser_url_len]=0;redraw();}return;}
...
void desktop_init(OrionBootInfo *bi){
    g_bi=bi;(void)g_bi;wins[APP_TERMINAL]=(Window){280,130,720,480,1,0,11,"Terminal"};wins[APP_FILES]=(Window){230,115,700,440,0,0,2,"Files"};wins[APP_NOTES]=(Window){330,125,610,480,0,0,3,"Notes"};wins[APP_PAINT]=(Window){250,90,760,560,0,0,4,"Paint"};wins[APP_BROWSER]=(Window){220,85,820,590,0,0,5,"Orion Browser"};wins[APP_NETWORK]=(Window){345,135,620,430,0,0,6,"Network"};wins[APP_ABOUT]=(Window){380,165,540,360,0,0,7,"About UN_Orion"};active_app=APP_TERMINAL;ztop=11;term_push("Welcome to UN_Orion Desktop 0.0.5.");term_push("Networking and Orion Browser are now built in.");term_push("Type 'help' for terminal commands.");redraw();
}

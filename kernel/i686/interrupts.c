...
static IdtGate32 idt[256];
static uint64_t gdt[3]={0,0x00CF9A000000FFFFULL,0x00CF92000000FFFFULL};
extern void orion_load_gdt(struct GdtPtr32 *p);
...
void arch_gdt_init(void){struct GdtPtr32 p={(uint16_t)(sizeof(gdt)-1),(uint32_t)(uintptr_t)gdt};orion_load_gdt(&p);}
static void idt_gate(int v,void *fn){uint32_t a=(uint32_t)(uintptr_t)fn;idt[v]=(IdtGate32){(uint16_t)a,0x08,0,0x8e,(uint16_t)(a>>16)};}
...
static void pit_program(uint32_t hz){
    ...
    uint32_t divisor=PIT_BASE_HZ/hz;if(!divisor)divisor=1;if(divisor>65535)divisor=65535;
    g_pit_hz=PIT_BASE_HZ/divisor;outb(PIT_CMD,0x36);outb(PIT_CH0,(uint8_t)divisor);outb(PIT_CH0,(uint8_t)(divisor>>8));
}
__attribute__((interrupt)) static void pit_isr(struct I686IntFrame*f){(void)f;g_ticks++;outb(PIC1,0x20);}
__attribute__((interrupt)) static void keyboard_isr(struct I686IntFrame*f){(void)f;uint8_t s=inb(0x60),n=(uint8_t)(key_head+1)&63;if(n!=key_tail){keyq[key_head]=s;key_head=n;}outb(PIC1,0x20);}
__attribute__((interrupt)) static void mouse_isr(struct I686IntFrame*f){(void)f;uint8_t b=inb(0x60),n=(uint8_t)(mouse_head+1)&127;if(n!=mouse_tail){mouseq[mouse_head]=b;mouse_head=n;}outb(PIC2,0x20);outb(PIC1,0x20);}
...
static void mouse_init(void){
    ...
    if(!ps2_wait_read())return;uint8_t cmd=inb(0x60);cmd|=2;cmd&=(uint8_t)~0x20;
    if(!ps2_wait_write())return;outb(0x64,0x60);if(!ps2_wait_write())return;outb(0x60,cmd);
    if(!mouse_write(0xF6)||!mouse_write(0xF4))return;g_mouse_available=1;
}
void interrupts_init(uint32_t pit_hz){
    for(int i=0;i<256;i++)idt[i]=(IdtGate32){0};
    ...
    struct IdtPtr32 p={(uint16_t)(sizeof(idt)-1),(uint32_t)(uintptr_t)idt};__asm__ volatile("lidt %0"::"m"(p));
...
int keyboard_pop_scancode(uint8_t*s){if(key_tail==key_head)return 0;if(s)*s=keyq[key_tail];key_tail=(uint8_t)(key_tail+1)&63;return 1;}
int mouse_pop_byte(uint8_t*b){if(mouse_tail==mouse_head)return 0;if(b)*b=mouseq[mouse_tail];mouse_tail=(uint8_t)(mouse_tail+1)&127;return 1;}
...
__attribute__((noreturn)) void orion_exception_panic32(uint32_t v,uint32_t e,uint32_t ip,uint32_t cs,uint32_t fl){
    (void)cs;(void)fl;__asm__ volatile("cli");serial_write("\r\nUN_Orion i686 PANIC: ");serial_write(exname(v));serial_write(" vector=");serial_write_hex64(v);serial_write(" error=");serial_write_hex64(e);serial_write(" eip=");serial_write_hex64(ip);serial_write("\r\n");
    if(gfx_ready()){gfx_rect(0,0,gfx_width(),gfx_height(),0x13090D);gfx_display_text(42,76,"PANIC",0xFFE7EC);gfx_text(44,150,exname(v),0xFF8FA3,1);gfx_text(44,214,"i686 / Legacy BIOS",0xB77A89,1);}

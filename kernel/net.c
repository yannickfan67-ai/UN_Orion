...
static uint32_t tcp_iss,tcp_snd_nxt,tcp_snd_una,tcp_rcv_nxt;
...
static size_t dns_skip_name(const uint8_t*d,size_t n,size_t o){unsigned labels=0;while(o<n&&labels++<128){uint8_t z=d[o];if(z==0)return o+1;if((z&0xc0u)==0xc0u)return o+2;if((z&0xc0u)||z>63u||o+1u+(size_t)z>n)return n+1;o+=1u+(size_t)z;}return n+1;}
...
static void handle_tcp(const uint8_t*p,size_t len,const uint8_t srcip[4]){
    if(len<20||!memequal(srcip,tcp_peer_ip,4))return;uint16_t sp=get16(p),dp=get16(p+2);if(sp!=tcp_peer_port||dp!=tcp_local_port)return;uint32_t seq=get32(p+4),ack=get32(p+8);uint8_t hlen=(p[12]>>4)*4,flags=p[13];if(hlen<20||hlen>len)return;if((flags&0x10)&&ack>=tcp_snd_una&&ack<=tcp_snd_nxt)tcp_snd_una=ack;
...
static int http_status_code(const char*raw,size_t len){
...
/* 1=resolved, -1=refused HTTPS downgrade, 0=unsupported/malformed. */

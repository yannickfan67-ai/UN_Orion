...
int net_http_get(const char *url,char *body,size_t cap,char *status,size_t status_cap){
    ...
    char req[512];size_t o=0;const char*a="GET ";while(*a&&o+1<sizeof(req))req[o++]=*a++;for(size_t i=0;path[i]&&o+1<sizeof(req);i++)req[o++]=path[i];a=" HTTP/1.0\r\nHost: ";while(*a&&o+1<sizeof(req))req[o++]=*a++;for(size_t i=0;host[i]&&o+1<sizeof(req);i++)req[o++]=host[i];a="\r\nUser-Agent: OrionBrowser/0.0.5\r\nConnection: close\r\n\r\n";while(*a&&o+1<sizeof(req))req[o++]=*a++;send_tcp(0x18,(const uint8_t*)req,(uint16_t)o);tcp_event=2;st=now_ms();uint32_t last_data=st;while(!elapsed(st,8000)){net_poll();if(tcp_event==3){last_data=now_ms();tcp_event=2;}if(tcp_event==4||tcp_event<0)break;if(tcp_out_len&&elapsed(last_data,1200))break;__asm__ volatile("pause");}raw[tcp_out_len]=0;if(!tcp_out_len){scopy(status,status_cap,"No HTTP data");return 0;}
    char *sep=0;for(size_t i=0;i+3<tcp_out_len;i++)if(raw[i]=='\r'&&raw[i+1]=='\n'&&raw[i+2]=='\r'&&raw[i+3]=='\n'){sep=raw+i+4;break;}if(status&&status_cap){size_t n=0;while(n<tcp_out_len&&raw[n]!='\r'&&raw[n]!='\n'&&n+1<status_cap){status[n]=raw[n];n++;}status[n]=0;}const char *src=sep?sep:raw;size_t bi=0;int intag=0,last_space=0;for(size_t i=0;src[i]&&bi+1<cap;i++){char c=src[i];if(c=='<'){intag=1;if(!last_space&&bi){body[bi++]=' ';last_space=1;}continue;}if(c=='>'){intag=0;continue;}if(intag)continue;if(c=='\r')continue;if(c=='\n'||c=='\t')c=' ';if(c=='&'){/* minimal entities */if(src[i+1]=='n'&&src[i+2]=='b'&&src[i+3]=='s'&&src[i+4]=='p'&&src[i+5]==';'){c=' ';i+=5;}else if(src[i+1]=='a'&&src[i+2]=='m'&&src[i+3]=='p'&&src[i+4]==';'){c='&';i+=4;}}if(c==' '){if(last_space)continue;last_space=1;}else last_space=0;if((unsigned char)c>=32)body[bi++]=c;}body[bi]=0;serial_write("HTTP: response received\r\n");return 1;
}
